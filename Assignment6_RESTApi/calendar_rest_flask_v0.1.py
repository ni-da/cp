...
class Appointment:
    def __init__(self, id=None, title='', location='', description='', start=None, stop=None):
...
# generates a new appointment
# /appointments[/][?format=json|xml]
...
def create_appointment():
    ...
    generated_id = id_generate()
    a = Appointment(id=generated_id,
                    title=args['title'],
                    location=args['location'],
                    description=args['description'],
                    start=args['start'], stop=args['stop'])
    if a.start is None: a.start = datetime.now()
    if a.stop is None: a.stop = datetime.now()
    calApp = Calendar.add_appointment(cal, generated_id, a.to_json())
    return calApp, 201, {'Content-Type': 'JSON/XML; charset=utf-8'}


# GET /appointments/<id_appointment>[?format=json|xml]
# returns the specified appointment in the requested representation format.
@app.route('/appointments/<id_appointment>', methods=['GET', 'DELETE', 'PUT'])
@app.route('/appointments/<id_appointment>/<format>', methods=['GET'])
def get_appointment(id_appointment, format='json | xml'):
    if request.method == 'GET':
        appointment = cal.get_appointment(int(id_appointment))
        if appointment is None:
            return jsonify(errorJson), 400, {'Content-Type': 'JSON/XML; charset=utf-8'}
        return appointment, 201, {'Content-Type': 'JSON/XML; charset=utf-8'}
    elif request.method == 'DELETE':
        # DELETE /appointments/<id_appointment> deletes the specified appointment;
        appointment = cal.delete_appointment(int(id_appointment))
        if appointment is None:
            return jsonify(errorJson), 400, {'Content-Type': 'JSON/XML; charset=utf-8'}
        ...
        old_appointment = cal.get_appointment(int(id_appointment))

        appointment = cal.update_appointment(int(id_appointment), old_appointment)
        apo = Appointment(title="")
        apo.from_json(appointment)
        parser = reqparse.RequestParser()
        parser.add_argument("title")
        parser.add_argument("location")
        parser.add_argument("description")
        ...
        args = parser.parse_args()
        if 'title' in args:
            apo.title = args['title']
        if 'location' in args:
            apo.location = args['location']
        if 'description' in args:
            apo.description = args['description']
        if 'start' in args:
            apo.start = args['start']
        if 'stop' in args:
            apo.stop = args['stop']
        updated_appointment = cal.update_appointment(apo.id, apo)
