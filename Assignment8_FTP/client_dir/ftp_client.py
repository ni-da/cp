...
class FTPclient:
    def __init__(self, address, port, data_port, username, password):
        ...
        self.ascii_mode = ['ms', 'txt', 'html', 'src']
            ...
            if server_msg[:3] == '220':  # Service ready for new user.
                ...
                if server_msg[:3] == '331':  # User name okay, need password.
                    ...
                    if server_msg[:3] == '230':  # User logged in, proceed. Logged out if appropriate.
                        self.control_sock.sendall('STRU F\r\n') #default structure is File, maar voor de zekerheid
                        file_struc = (self.control_sock.recv(1024)) #200 F OK
                        self.control_sock.sendall('MODE S\r\n')  #  default transfer mode is Stream, maar voor de zekerheid
                        stream_msg = (self.control_sock.recv(1024))  # 200 S OK
                        self.control_sock.send('TYPE I\r\n')
                        img_msg = self.control_sock.recv(1024)  # 200 TYPE is now 8-bit binary
    ...
    def STOR(self, filename):
        if filename.split('.')[1] in self.ascii_mode:
            self.control_sock.send('TYPE A\r\n')
            ascii_msg = self.control_sock.recv(1024)  # 200 TYPE is now 8-bit binary
        else:
            self.control_sock.send('TYPE I\r\n')
            img_msg = self.control_sock.recv(1024)  # 200 TYPE is now 8-bit binary
        try:
